      ...
      fun (ofmt : Format.formatter) arg →
        (fun ofmt
           ...
           pf ofmt
             ...
             (let open Pa_ppx_runtime.Runtime in Fmt.bool) v_loc_ghost)
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
  ...
  struct
    type t =
      Longident.t ==
        ...
        | Lapply of Location.loc t and Location.loc t ][@@"deriving_inline" show;]
    ;
    value rec pp : Fmt.t t =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | Ldot v0 v1 →
               let open Pa_ppx_runtime.Runtime.Fmt in
               pf ofmt "(@[<2>Longident.Ldot@ (@,%a,@ %a@,))@]"
                 (Location.pp_loc pp) v0
                 (Location.pp_loc
                    (fun ofmt arg →
                       let open Pa_ppx_runtime.Runtime.Fmt in
                       pf ofmt "%S" arg))
           ...
           | Lapply v0 v1 →
               let open Pa_ppx_runtime.Runtime.Fmt in
               pf ofmt "(@[<2>Longident.Lapply@ (@,%a,@ %a@,))@]"
                 (Location.pp_loc pp) v0 (Location.pp_loc pp) v1 ])
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    and show : t → Stdlib.String.t =
      fun arg → Format.asprintf "%a" pp arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    ;
...
module Asttypes =
  struct
    ...
    type variance =
      Asttypes.variance ==
        [ Covariant | Contravariant | NoVariance | Bivariant ][@@"deriving_inline" show;]
    ;
...
and core_type_desc =
  Parsetree.core_type_desc ==
    ...
    | Ptyp_tuple of list (option string * core_type)
    ...
    | Ptyp_functor of
        Asttypes.arg_label and Asttypes.loc string and package_type and
          core_type ]
and package_type =
  Parsetree.package_type ==
    { ppt_path : Asttypes.loc Longident.t;
      ppt_constraints : list (Asttypes.loc Longident.t * core_type);
      ppt_loc : Location.t;
...
and pattern_desc =
  Parsetree.pattern_desc ==
    ...
    | Ppat_tuple of list (option string * pattern) and Asttypes.closed_flag
    | Ppat_construct of
    ...
    | Ppat_unpack of Asttypes.loc (option string) and option package_type
...
and expression_desc =
  Parsetree.expression_desc ==
    ...
    | Pexp_tuple of list (option string * expression)
...
and type_declaration =
  Parsetree.type_declaration ==
    { ptype_name : Asttypes.loc string;
      ...
      ptype_constraints : list (core_type * core_type * Location.t);
...
and pp_core_type_desc : Fmt.t core_type_desc =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | Ptyp_functor v0 v1 v2 v3 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Ptyp_functor@ (@,%a,@ %a,@ %a,@ %a@,))@]"
             Asttypes.pp_arg_label v0
             (Asttypes.pp_loc
                (fun ofmt arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
             v1 pp_package_type v2 pp_core_type v3 ])
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
...
and pp_package_type : Fmt.t package_type =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt
         ({ppt_path = v_ppt_path; ppt_constraints = v_ppt_constraints;
           ppt_loc = v_ppt_loc; ppt_attrs = v_ppt_attrs} :
          package_type) →
       let open Pa_ppx_runtime.Runtime.Fmt in
       pf ofmt
         "@[<2>{ @[Parsetree.ppt_path =@ %a@];@ @[ppt_constraints =@ %a@];@ @[ppt_loc =@ %a@];@ @[ppt_attrs =@ %a@] }@]"
         (Asttypes.pp_loc Longident.pp) v_ppt_path
         (fun (ofmt : Format.formatter) arg →
            let open Pa_ppx_runtime.Runtime.Fmt in
            pf ofmt "@[<2>[%a@,]@]"
              (list ~{sep = semi}
                 (fun (ofmt : Format.formatter) (v0, v1) →
                    ...
                    pf ofmt "(@[%a,@ %a@])" (Asttypes.pp_loc Longident.pp) v0
                      pp_core_type v1))
              arg)
         v_ppt_constraints Location.pp v_ppt_loc pp_attributes v_ppt_attrs)
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    ...
    (fun ofmt →
       ...
       [ Rtag v0 v1 v2 →
           ...
           pf ofmt "(@[<2>Parsetree.Rtag@ (@,%a,@ %a,@ %a@,))@]"
             ...
             (let open Pa_ppx_runtime.Runtime in Fmt.bool) v1
             (fun (ofmt : Format.formatter) arg →
                let open Pa_ppx_runtime.Runtime.Fmt in
                pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_core_type) arg)
             v2
...
and pp_pattern_desc : Fmt.t pattern_desc =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | Ppat_tuple v0 v1 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Ppat_tuple@ (@,%a,@ %a@,))@]"
             (fun (ofmt : Format.formatter) arg →
                let open Pa_ppx_runtime.Runtime.Fmt in
                pf ofmt "@[<2>[%a@,]@]"
                  (list ~{sep = semi}
       ...
       | Ppat_unpack v0 v1 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Ppat_unpack@ (@,%a,@ %a@,))@]"
             (Asttypes.pp_loc
                (fun ofmt →
                   fun
                   [ None →
             ...
             (fun ofmt →
                ...
                | Some arg →
                    let open Pa_ppx_runtime.Runtime.Fmt in
                    pf ofmt "(Some %a)" pp_package_type arg ])
             v1
       | Ppat_exception v0 →
           let open Pa_ppx_runtime.Runtime.Fmt in
...
and pp_expression_desc : Fmt.t expression_desc =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
           ...
           pf ofmt "(@[<2>Parsetree.Pexp_override@ %a)@]"
             (fun (ofmt : Format.formatter) arg →
                ...
                pf ofmt "@[<2>[%a@,]@]"
                  ...
                  arg)
             v0
       | Pexp_struct_item v0 v1 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Pexp_struct_item@ (@,%a,@ %a@,))@]"
             pp_structure_item v0 pp_expression v1
       | Pexp_assert v0 →
           let open Pa_ppx_runtime.Runtime.Fmt in
       ...
       | Pexp_pack v0 v1 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Pexp_pack@ (@,%a,@ %a@,))@]"
             pp_module_expr v0
             (fun ofmt →
                fun
                [ None →
  ...
  fun (ofmt : Format.formatter) arg →
    (fun ofmt
         ({ptype_name = v_ptype_name; ptype_params = v_ptype_params;
           ptype_constraints = v_ptype_constraints; ptype_kind = v_ptype_kind;
           ptype_private = v_ptype_private; ptype_manifest = v_ptype_manifest;
           ptype_attributes = v_ptype_attributes; ptype_loc = v_ptype_loc} :
          type_declaration) →
       let open Pa_ppx_runtime.Runtime.Fmt in
       pf ofmt
         "@[<2>{ @[Parsetree.ptype_name =@ %a@];@ @[ptype_params =@ %a@];@ @[ptype_constraints =@ %a@];@ @[ptype_kind =@ %a@];@ @[ptype_private =@ %a@];@ @[ptype_manifest =@ %a@];@ @[ptype_attributes =@ %a@];@ @[ptype_loc =@ %a@] }@]"
         (Asttypes.pp_loc
            (fun ofmt arg →
               let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
         v_ptype_name
              ...
              (list ~{sep = semi}
                 (fun (ofmt : Format.formatter) (v0, v1, v2) →
                    ...
                    pf ofmt "(@[%a,@ %a,@ %a@])" pp_core_type v0 pp_core_type
                      v1 Location.pp v2))
              arg)
         v_ptype_constraints pp_type_kind v_ptype_kind
         Asttypes.pp_private_flag v_ptype_private
         (fun ofmt →
            fun
            [ None →
                let open Pa_ppx_runtime.Runtime.Fmt in
...
and pp_type_kind : Fmt.t type_kind =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | Ptype_external v0 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>Parsetree.Ptype_external@ %a)@]"
             (fun ofmt arg →
                let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg)
             v0 ])
      ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]

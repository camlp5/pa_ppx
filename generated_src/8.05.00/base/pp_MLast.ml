...
[@@@"end"];
type type_var =
  (Ploc.vala (option string) * Ploc.vala string)[@@"deriving_inline" show;]
;
...
and ctyp =
  MLast.ctyp ==
    ...
    | TyTup of
        loc and
          Ploc.vala (list (Ploc.vala (option (Ploc.vala string)) * ctyp))
    | TyVrn of
...
and patt =
  MLast.patt ==
    ...
    | PaLab of loc and patt and Ploc.vala (option patt)
    ...
    | PaTup of loc and Ploc.vala (list patt) and Ploc.vala bool
...
and expr =
  MLast.expr ==
    ...
    | ExLab of loc and patt and Ploc.vala (option expr)
    ...
    | ExLSI of loc and Ploc.vala str_item and expr
    | ExUnr of loc ]
...
and sig_item =
  MLast.sig_item ==
    ...
    | SgMtySubst of loc and Ploc.vala string and module_type and attributes
    | SgMtyAlias of
...
and generic_constructor =
  (loc * Ploc.vala string * Ploc.vala (list string) * Ploc.vala (list ctyp) *
   Ploc.vala (option ctyp) * attributes)
  ...
  fun (type a) (tp_0 : Fmt.t a) (ofmt : Format.formatter) arg →
    (fun ofmt
       ...
       pf ofmt
         ...
         pp_loc v_ciLoc
         (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool)) v_ciVir
         (fun (ofmt : Format.formatter) (v0, v1) →
            let open Pa_ppx_runtime.Runtime.Fmt in
    ...
    (fun ofmt →
       ...
       | TyMan v0 v1 v2 v3 →
           ...
           pf ofmt "(@[<2>MLast.TyMan@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc v0
             pp_ctyp v1
             (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool)) v2
             pp_ctyp v3
       | TyObj v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
             ...
             (Ploc.pp_vala
                (fun (ofmt : Format.formatter) arg →
                   ...
                   pf ofmt "@[<2>[%a@,]@]"
                     ...
                     arg))
             v1 (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool))
             v2
       | TyOlb v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
                        ...
                        (fun (ofmt : Format.formatter) (v0, v1, v2, v3, v4) →
                           ...
                           pf ofmt "(@[%a,@ %a,@ %a,@ %a,@ %a@])" pp_loc v0
                             (fun ofmt arg →
                                let open Pa_ppx_runtime.Runtime.Fmt in
                                pf ofmt "%S" arg)
                             v1 (let open Pa_ppx_runtime.Runtime in Fmt.bool)
       ...
       | TyExt v0 v1 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.TyExt@ (@,%a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala
                (fun ofmt arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
             v1
...
and pp_patt : Fmt.t patt =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | PaLong v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.PaLong@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             pp_longid v1
             (Ploc.pp_vala
                (fun (ofmt : Format.formatter) arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in
       ...
       | PaEff v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.PaEff@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             pp_patt v1 pp_patt v2
       | PaFlo v0 v1 →
           let open Pa_ppx_runtime.Runtime.Fmt in
       ...
       | PaLab v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.PaLab@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             pp_patt v1
             (Ploc.pp_vala
                (fun ofmt →
                   fun
       ...
       | PaTup v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.PaTup@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala
                (fun (ofmt : Format.formatter) arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in
                   pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_patt) arg))
    ...
    (fun ofmt →
       ...
       | ExFor v0 v1 v2 v3 v4 v5 →
           ...
           pf ofmt "(@[<2>MLast.ExFor@ (@,%a,@ %a,@ %a,@ %a,@ %a,@ %a@,))@]"
             ...
             (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool)) v4
             (Ploc.pp_vala
                (fun (ofmt : Format.formatter) arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in
                   pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_expr) arg))
       ...
       | ExLab v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.ExLab@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             pp_patt v1
             (Ploc.pp_vala
                (fun ofmt →
                   fun
       ...
       | ExLet v0 v1 v2 v3 →
           ...
           pf ofmt "(@[<2>MLast.ExLet@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool)) v1
             (Ploc.pp_vala
                (fun (ofmt : Format.formatter) arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in
                   pf ofmt "@[<2>[%a@,]@]"
       ...
       | ExLSI v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.ExLSI@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala pp_str_item) v1 pp_expr v2
       | ExUnr v0 →
           let open Pa_ppx_runtime.Runtime.Fmt in
...
and pp_sig_item : Fmt.t sig_item =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | SgExt v0 v1 v2 v3 v4 v5 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.SgExt@ (@,%a,@ %a,@ %a,@ %a,@ %a,@ %a@,))@]"
             pp_loc v0
             (Ploc.pp_vala
                (fun ofmt arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
       ...
       | SgMtySubst v0 v1 v2 v3 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.SgMtySubst@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
             v0
             (Ploc.pp_vala
                (fun ofmt arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
...
and pp_with_constr : Fmt.t with_constr =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | WcMty v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.WcMty@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala pp_longid) v1 pp_module_type v2
       | WcMts v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.WcMts@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala pp_longid) v1 pp_module_type v2
           ...
           pf ofmt "(@[<2>MLast.WcTyp@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]" pp_loc
             ...
             (Ploc.pp_vala
                (fun (ofmt : Format.formatter) arg →
                   ...
                   pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_type_var)
                     arg))
             v2 (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool))
             v3 pp_ctyp v4
       | WcTys v0 v1 v2 v3 →
           let open Pa_ppx_runtime.Runtime.Fmt in
...
and pp_str_item : Fmt.t str_item =
  fun (ofmt : Format.formatter) arg →
    (fun ofmt →
       ...
       | StExt v0 v1 v2 v3 v4 v5 →
           let open Pa_ppx_runtime.Runtime.Fmt in
           pf ofmt "(@[<2>MLast.StExt@ (@,%a,@ %a,@ %a,@ %a,@ %a,@ %a@,))@]"
             pp_loc v0
             (Ploc.pp_vala
                (fun ofmt arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
       ...
       | StOpn v0 v1 v2 v3 →
           ...
           pf ofmt "(@[<2>MLast.StOpn@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc v0
             (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool)) v1
             pp_module_expr v2 pp_attributes v3
       | StTyp v0 v1 v2 →
           let open Pa_ppx_runtime.Runtime.Fmt in
  ...
  fun (ofmt : Format.formatter) arg →
    (fun ofmt
       ...
       pf ofmt
         ...
         (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool))
         v_tdIsDecl
         (Ploc.pp_vala
            (fun (ofmt : Format.formatter) (v0, v1) →
               let open Pa_ppx_runtime.Runtime.Fmt in
         ...
         v_tdPrm (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool))
         v_tdPrv pp_ctyp v_tdDef
         (Ploc.pp_vala
            (fun (ofmt : Format.formatter) arg →
               let open Pa_ppx_runtime.Runtime.Fmt in
               pf ofmt "@[<2>[%a@,]@]"
...
and pp_generic_constructor : Fmt.t generic_constructor =
  fun (ofmt : Format.formatter) arg →
    (fun (ofmt : Format.formatter) (v0, v1, v2, v3, v4, v5) →
       let open Pa_ppx_runtime.Runtime.Fmt in
       pf ofmt "(@[%a,@ %a,@ %a,@ %a,@ %a,@ %a@])" pp_loc v0
         (Ploc.pp_vala
            (fun ofmt arg →
               let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
         v1
       ...
       pf ofmt
         ...
         (Ploc.pp_vala
            (fun (ofmt : Format.formatter) arg →
               let open Pa_ppx_runtime.Runtime.Fmt in
               pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_type_var) arg))
         v_tePrm (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool))
         v_tePrv
         (Ploc.pp_vala
            (fun (ofmt : Format.formatter) arg →
               let open Pa_ppx_runtime.Runtime.Fmt in
    ...
    (fun ofmt →
       ...
       | CgMth v0 v1 v2 v3 v4 →
           ...
           pf ofmt "(@[<2>MLast.CgMth@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]" pp_loc
             v0 (Ploc.pp_vala (let open Pa_ppx_runtime.Runtime in Fmt.bool))
             v1
             (Ploc.pp_vala
                (fun ofmt arg →
                   let open Pa_ppx_runtime.Runtime.Fmt in pf ofmt "%S" arg))
...
;
[@@@"end"];
Pp_debug.Pp_MLast.ref_pp_longid.val := pp_longid;
Pp_debug.Pp_MLast.ref_pp_longid_lident.val := pp_longid_lident;
Pp_debug.Pp_MLast.ref_pp_ctyp.val := pp_ctyp;
Pp_debug.Pp_MLast.ref_pp_expr.val := pp_expr;
Pp_debug.Pp_MLast.ref_pp_patt.val := pp_patt;
Pp_debug.Pp_MLast.ref_pp_loc.val := pp_loc;
Pp_debug.Pp_MLast.ref_pp_type_var.val := pp_type_var;
Pp_debug.Pp_MLast.ref_pp_longid.val := pp_longid;
Pp_debug.Pp_MLast.ref_pp_ctyp.val := pp_ctyp;
Pp_debug.Pp_MLast.ref_pp_poly_variant.val := pp_poly_variant;
Pp_debug.Pp_MLast.ref_pp_patt.val := pp_patt;
Pp_debug.Pp_MLast.ref_pp_expr.val := pp_expr;
Pp_debug.Pp_MLast.ref_pp_case_branch.val := pp_case_branch;
Pp_debug.Pp_MLast.ref_pp_module_type.val := pp_module_type;
Pp_debug.Pp_MLast.ref_pp_functor_parameter.val := pp_functor_parameter;
Pp_debug.Pp_MLast.ref_pp_sig_item.val := pp_sig_item;
Pp_debug.Pp_MLast.ref_pp_with_constr.val := pp_with_constr;
Pp_debug.Pp_MLast.ref_pp_module_expr.val := pp_module_expr;
Pp_debug.Pp_MLast.ref_pp_str_item.val := pp_str_item;
Pp_debug.Pp_MLast.ref_pp_type_decl.val := pp_type_decl;
Pp_debug.Pp_MLast.ref_pp_generic_constructor.val := pp_generic_constructor;
Pp_debug.Pp_MLast.ref_pp_extension_constructor.val := pp_extension_constructor;
Pp_debug.Pp_MLast.ref_pp_type_extension.val := pp_type_extension;
Pp_debug.Pp_MLast.ref_pp_class_type.val := pp_class_type;
Pp_debug.Pp_MLast.ref_pp_class_sig_item.val := pp_class_sig_item;
Pp_debug.Pp_MLast.ref_pp_class_expr.val := pp_class_expr;
Pp_debug.Pp_MLast.ref_pp_class_str_item.val := pp_class_str_item;
Pp_debug.Pp_MLast.ref_pp_longid_lident.val := pp_longid_lident;
Pp_debug.Pp_MLast.ref_pp_payload.val := pp_payload;
Pp_debug.Pp_MLast.ref_pp_attribute_body.val := pp_attribute_body;
Pp_debug.Pp_MLast.ref_pp_attribute.val := pp_attribute;
Pp_debug.Pp_MLast.ref_pp_attributes_no_anti.val := pp_attributes_no_anti;
Pp_debug.Pp_MLast.ref_pp_attributes.val := pp_attributes;

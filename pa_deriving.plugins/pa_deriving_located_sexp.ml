...
module PM = ParamMap(struct value arg_ctyp_f loc pty = <:ctyp< $pty$ -> Pa_ppx_located_sexp.Sexp.t >> ; end) ;

value to_sexp_fname arg tyname =
  (*if tyname = "t" then "sexp_of"
    else *)"located_sexp_of_"^tyname
;
...
value omission_instructions loc arg attrs =
  match (extract_allowed_attribute_expr arg ("located_sexp", "default") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "sexp_drop_default") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "sexp_drop_default.ord") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "sexp_drop_default.eq") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "sexp_drop_default.sexp") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "sexp_drop_if") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "omit_nil") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "option") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "list") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "array") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "bool") attrs
        ) with [
      (None, None, None, None, None, None, None, None, None, None, None) -> None
    | (Some d, None, None, None, None, None, None, None, None, None, None) -> None
    | (Some d, Some <:expr< () >>, None, None, None, None, None, None, None, None, None) -> Some (Default (Builtin_eq d))
    | (Some d, Some eqfun, None, None, None, None, None, None, None, None, None) -> Some (Default (Code eqfun d))
    | (Some d, None, Some <:expr:< () >>, None, None, None, None, None, None, None, None) -> Some (Default (Compare d))
    | (Some d, None, None, Some <:expr:< () >>, None, None, None, None, None, None, None) -> Some (Default (Eq d))
    | (Some d, None, None, None, Some <:expr:< () >>, None, None, None, None, None, None) -> Some (Default (Sexp d))
    | (_, None, None, None, None, Some pred, None, None, None, None, None) -> Some (Default (If pred))
    | (_, None, None, None, None, None, Some <:expr< () >>, None, None, None, None) -> Some Omit_nil
    | (_, None, None, None, None, None, None, Some <:expr< () >>, None, None, None) -> Some Option
...
value to_expression arg ?{coercion} ~{msg} param_map ty0 =
  ...
  <:ctyp:< $t$ [@ $attrid:(_, id)$ ] >> when Some id = DC.allowed_attribute (DC.get arg) "located_sexp" "opaque" ->
    <:expr< fun _ -> Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy "<opaque>" >>
...
| <:ctyp:< unit >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_unit Ploc.dummy >>
| <:ctyp:< int >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_int Ploc.dummy >>
| <:ctyp:< bool >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_bool Ploc.dummy >>
| <:ctyp:< int32 >> | <:ctyp:< Int32.t >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_int32 Ploc.dummy >>
| <:ctyp:< int64 >> | <:ctyp:< Int64.t >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_int64 Ploc.dummy >>
| (<:ctyp:< string >> | <:ctyp:< Stdlib.String.t >> | <:ctyp:< String.t >>) ->
  <:expr< Pa_ppx_located_sexp.Conv.sexp_of_string Ploc.dummy >>
| <:ctyp:< bytes >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_bytes Ploc.dummy >>
| <:ctyp:< char >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_char Ploc.dummy >>
| <:ctyp:< nativeint >> | <:ctyp:< Nativeint.t >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_nativeint Ploc.dummy >>
| <:ctyp:< float >> -> <:expr< Pa_ppx_located_sexp.Conv.sexp_of_float Ploc.dummy >>

| <:ctyp:< Hashtbl.t >> ->
  <:expr< Pa_ppx_located_sexp.Conv.sexp_of_hashtbl Ploc.dummy >>

| <:ctyp:< $t$ [@ $attrid:(_, id)$ ] >> when Some id = DC.allowed_attribute (DC.get arg) "located_sexp" "nobuiltin" ->
    fmtrec ~{attrmod=Some Nobuiltin} t

| <:ctyp:< $t$ [@ $attrid:(_, id)$ $exp:e$ ;] >> when Some id = DC.allowed_attribute (DC.get arg) "located_sexp" "located_sexp_of" ->
    e
...
| <:ctyp:< list $ty$ >> ->
  ...
  <:expr< Pa_ppx_located_sexp.Conv.sexp_of_list Ploc.dummy $fmt1$ >>

| <:ctyp:< array $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.sexp_of_array Ploc.dummy $fmt1$ >>

| (<:ctyp:< ref $ty$ >> | <:ctyp:< Stdlib.ref $ty$ >>) ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.sexp_of_ref Ploc.dummy $fmt1$ >>

| <:ctyp:< option $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.sexp_of_option Ploc.dummy $fmt1$ >>
...
| <:ctyp:< [ $list:l$ ] >> ->
  let branches = List.map (fun [
    ...
    let jscid = match extract_allowed_attribute_expr arg ("located_sexp", "name") (uv attrs) with [
    ...
    let body_liste = match body with [
          <:expr< Pa_ppx_located_sexp.Sexp.List Ploc.dummy $liste$ >> -> liste
        | _ -> failwith "internal error handling inline records"
        ] in

    let conspat = <:patt< $uid:cid$ $recpat$ >> in
    (conspat, <:vala< None >>, <:expr< Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ (Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jscid$) :: $body_liste$ ] >>)
  ...
  | <:constructor:< $uid:cid$ of $list:tyl$ $_algattrs:attrs$ >> ->
    ...
    let consexp =
      match (tyl, vars, fmts) with [
          (_,[],[]) -> <:expr< Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jscid$ >>

        | ([<:ctyp< list $ty$ >>],[v],[_])
               when None <> extract_allowed_attribute_expr arg ("located_sexp", "list") (uv attrs) ->
           let fmtf = fmtrec ty in
           let liste = <:expr< List.map $fmtf$ $lid:v$ >> in
           <:expr< Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ (Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jscid$) :: $liste$ ] >>
...
| <:ctyp:< [= $list:l$ ] >> ->
  let branches = List.map (fun [
    ...
    let e =
      match (tyl, vars, fmts) with [
          (_, [], []) ->
          <:expr< Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jscid$ >>
        ...
        | (_, [v],[f]) ->
           <:expr< Pa_ppx_located_sexp.Sexp.List Ploc.dummy [Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jscid$ ; $f$ $lid:v$ ] >>
        | _ ->
           let liste = List.fold_right2 (fun f v liste -> <:expr< [$f$ $lid:v$ :: $liste$] >>)
                         fmts vars <:expr< [] >> in
           <:expr< Pa_ppx_located_sexp.Sexp.List Ploc.dummy [Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jscid$ ; Pa_ppx_located_sexp.Sexp.List Ploc.dummy $liste$] >>
        ] in
    (conspat, <:vala< None >>, e)
  }
...
| <:ctyp:< ( $list:tyl$ ) >> ->
    ...
    let liste = List.fold_right2 (fun f v liste -> <:expr< [$f$ $lid:v$ :: $liste$] >>)
        fmts vars <:expr< [] >> in
    let varpats = List.map (fun v -> <:patt< $lid:v$ >>) vars in
    <:expr< fun ($list:varpats$) -> Pa_ppx_located_sexp.Sexp.List Ploc.dummy $liste$ >>
...
and fmt_record loc arg fields = 
  let labels_vars_tys_fmts_omits_jskeys = List.map (fun (_, fname, _, ty, attrs) ->
        ...
        let key = extract_allowed_attribute_expr arg ("located_sexp", "key") attrs in
  ...
  let liste = List.fold_right (fun (f,v, ty,fmtf, omit, jskey) rhs ->
      match (fst (Ctyp.unwrap_attrs ty), omit) with [
        (_, Some (Default (Code eqcmpexp d))) -> <:expr< let fields = if $eqcmpexp$ $lid:v$ $d$ then fields
                           else [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; $fmtf$ $lid:v$ ] :: fields ] in $rhs$ >>
      | (_, Some (Default (Sexp d))) -> <:expr< let fmtv = $fmtf$ $lid:v$ in
                          let fmtd = $fmtf$ $d$ in
                         let fields = if Pa_ppx_located_sexp.Sexp.equal fmtv fmtd then fields
                           else [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; fmtv ] :: fields ] in $rhs$ >>
      ...
      | (_, Some Omit_nil) -> <:expr< let fmtv = $fmtf$ $lid:v$ in
                         let fields = if Pa_ppx_located_sexp.Sexp.equal fmtv (Pa_ppx_located_sexp.Sexp.List Ploc.dummy []) then fields
                           else [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; fmtv ] :: fields ] in $rhs$ >>
      ...
      | (<:ctyp< option $ty$ >>, Some Option) ->
         ...
         <:expr< let fields = match $lid:v$ with [
                       None -> fields
                     | Some v -> [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; $fmtf$ v ] :: fields ]
                     ] in $rhs$ >>

      | (<:ctyp< list $ty$ >>, Some List) ->
         <:expr< let fields = match $lid:v$ with [
                       [] -> fields
                     | _ -> [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; $fmtf$ $lid:v$ ] :: fields ]
                     ] in $rhs$ >>

      | (<:ctyp< array $ty$ >>, Some Array) ->
         <:expr< let fields = match $lid:v$ with [
                       [||] -> fields
                     | _ -> [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; $fmtf$ $lid:v$ ] :: fields ]
                     ] in $rhs$ >>
      ...
      | (<:ctyp< bool >>, Some Bool) ->
         <:expr< let fields = match $lid:v$ with [
                       False -> fields
                     | True -> [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ] :: fields ]
                     ] in $rhs$ >>

      | _ -> <:expr< let fields = [ Pa_ppx_located_sexp.Sexp.List Ploc.dummy [ Pa_ppx_located_sexp.Sexp.Atom Ploc.dummy $str:jskey$ ; $fmtf$ $lid:v$ ] :: fields ] in $rhs$ >>
      ]) (List.rev labels_vars_tys_fmts_omits_jskeys) <:expr< fields >> in
  let liste = <:expr< let fields = [] in $liste$ >> in

  let pl = List.map (fun (f, v, _, _, _, _) -> (<:patt< $lid:f$ >>, <:patt< $lid:v$ >>)) labels_vars_tys_fmts_omits_jskeys in
  (<:patt< { $list:pl$ } >>, <:expr< Pa_ppx_located_sexp.Sexp.List Ploc.dummy $liste$ >>)
...
value sig_item_top_funs arg td =
  ...
  let to_sexpfname = to_sexp_fname arg tyname in
  let paramtys = List.map (fun p -> <:ctyp< ' $PM.type_id p$ >>) param_map in
  let argfmttys = List.map (fun pty -> <:ctyp< $pty$ -> Pa_ppx_located_sexp.Sexp.t >>) paramtys in  
  let ty = <:ctyp< $lid:tyname$ >> in
  let toftype = Ctyp.arrows_list loc argfmttys <:ctyp< $(Ctyp.applist ty paramtys)$ -> Pa_ppx_located_sexp.Sexp.t >> in
...
| <:str_item:< type $lilongid:t$ $list:params$ += $_priv:_$ [ $list:ecs$ ] $_itemattrs:_$ >> ->
    ...
    let param_map = PM.make "located_sexp" loc params in
    let ec2gc = fun [
      EcTuple _ gc -> [gc]
    | EcRebind _ _ _ _ -> []
    ] in
...
module PM = ParamMap(struct value arg_ctyp_f loc pty = <:ctyp< Pa_ppx_located_sexp.Sexp.t -> $pty$ >> ; end) ;

value of_sexp_fname arg tyname =
  (*if tyname = "t" then "of_sexp"
    else*) tyname^"_of_located_sexp"
;
...
value omission_instructions loc arg attrs =
  match (extract_allowed_attribute_expr arg ("located_sexp", "option") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "list") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "array") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "bool") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "omit_nil") attrs,
         extract_allowed_attribute_expr arg ("located_sexp", "default") attrs
        ) with [
      (Some _, None, None, None, None, None) -> Option
    | (None, Some _, None, None, None, None) -> List
    | (None, None, Some _, None, None, None) -> Array
    | (None, None, None, Some _, None, None) -> Bool
    | (None, None, None, None, omit_nil_opt, default_opt) ->
       Omit { omit_nil = None <> omit_nil_opt ; default = default_opt }
    ]
;
...
| <:ctyp:< unit >> -> <:expr< Pa_ppx_located_sexp.Conv.unit_of_sexp >>
| <:ctyp:< int >> -> <:expr< Pa_ppx_located_sexp.Conv.int_of_sexp >>
| <:ctyp:< bool >> -> <:expr< Pa_ppx_located_sexp.Conv.bool_of_sexp >>
| <:ctyp:< int32 >> | <:ctyp:< Int32.t >> -> <:expr< Pa_ppx_located_sexp.Conv.int32_of_sexp >>
| <:ctyp:< int64 >> | <:ctyp:< Int64.t >> -> <:expr< Pa_ppx_located_sexp.Conv.int64_of_sexp >>
| (<:ctyp:< string >> | <:ctyp:< Stdlib.String.t >> | <:ctyp:< String.t >>) ->
  <:expr< Pa_ppx_located_sexp.Conv.string_of_sexp >>
| <:ctyp:< bytes >> -> <:expr< Pa_ppx_located_sexp.Conv.bytes_of_sexp >>
| <:ctyp:< char >> -> <:expr< Pa_ppx_located_sexp.Conv.char_of_sexp >>
| <:ctyp:< nativeint >> | <:ctyp:< Nativeint.t >> -> <:expr< Pa_ppx_located_sexp.Conv.nativeint_of_sexp >>
| <:ctyp:< float >> -> <:expr< Pa_ppx_located_sexp.Conv.float_of_sexp >>

| <:ctyp:< Hashtbl.t >> ->
  <:expr< Pa_ppx_located_sexp.Sexp.hashtbl_of_sexp >>

| <:ctyp:< $t$ [@ $attrid:(_, id)$ ] >> when Some id = DC.allowed_attribute (DC.get arg) "located_sexp" "nobuiltin" ->
    fmtrec ~{attrmod={ (attrmod) with nobuiltin = True }} t

| <:ctyp:< $t$ [@ $attrid:(_, id)$ ] >> when Some id = DC.allowed_attribute (DC.get arg) "located_sexp" "allow_extra_fields" ->
    fmtrec ~{attrmod={ (attrmod) with allow_extra_fields = True }} t

| <:ctyp:< $t$ [@ $attrid:(_, id)$ $exp:e$ ;] >> when Some id = DC.allowed_attribute (DC.get arg) "located_sexp" "of_located_sexp" ->
    e
...
| <:ctyp:< list $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.list_of_sexp $fmt1$ >>

| <:ctyp:< array $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.array_of_sexp $fmt1$ >>

| (<:ctyp:< ref $ty$ >> | <:ctyp:< Stdlib.ref $ty$ >>) ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.ref_of_sexp $fmt1$ >>

| <:ctyp:< option $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_sexp.Conv.option_of_sexp $fmt1$ >>
...
| <:ctyp:< [ $list:l$ ] >> ->
  ...
  | <:constructor:< $uid:cid$ of $list:tyl$ $_algattrs:attrs$ >> ->
    ...
    let jscid_pat =
      ...
      <:patt< (Pa_ppx_located_sexp.Sexp.Atom _ $str:jscid$)|(Pa_ppx_located_sexp.Sexp.Atom _ $str:uncap_jscid$) >> in
    ...
    let (conspat,consexp) =
      match (tyl, vars) with [
        ...
        | ([<:ctyp< list $ty$ >>],[v])
          when None <> extract_allowed_attribute_expr arg ("located_sexp", "list") (uv attrs) ->
           (<:patt< Pa_ppx_located_sexp.Sexp.List _ [ $jscid_pat$ :: $lid:v$ ] >>,
            <:expr< let $lid:v$ = Pa_ppx_located_sexp.Sexp.List Ploc.dummy $lid:v$ in
                    $consexp$ >>)

        | _ ->
           let conspatvars = List.fold_right (fun v rhs -> <:patt< [ $lid:v$ :: $rhs$ ] >>)
                               vars <:patt< [] >> in
           (<:patt< Pa_ppx_located_sexp.Sexp.List _ [ $jscid_pat$ :: $conspatvars$ ] >>,
            consexp)
        ] in
  ...
  let catch_branch = (<:patt< se >>, <:vala< None >>, <:expr< raise (Ploc.Exc (Pa_ppx_located_sexp.Sexp.loc_of_sexp se) (Failure $str:msg$)) >>) in
  let branches = branches @ [catch_branch] in
  <:expr< fun [ $list:branches$ ] >>
...
| <:ctyp:< [= $list:l$ ] >> as ty0 -> 
  ...
  let branches = List.map (fun [
    ...
    let (conspat, consexp) =
      match (tyl, vars) with [
          (_, []) ->
          (<:patt< Pa_ppx_located_sexp.Sexp.Atom _ $str:jscid$ >>, <:expr< ` $cid$ >>)                                    

        | ([<:ctyp< list $ty$ >>], [v])
          when None <> extract_allowed_attribute_expr arg ("located_sexp", "list") (uv attrs) ->
           (<:patt< Pa_ppx_located_sexp.Sexp.List _ [ Pa_ppx_located_sexp.Sexp.Atom _ $str:jscid$ :: $lid:v$ ] >>,
            <:expr< let $lid:v$ = Pa_ppx_located_sexp.Sexp.List Ploc.dummy $lid:v$
                    in ` $cid$ $tup$ >>)
        ...
        | _ ->
           let listpat = List.fold_right (fun vp listpat -> <:patt< [ $vp$ :: $listpat$ ] >>)
                           varpats <:patt< [] >> in
           (<:patt< Pa_ppx_located_sexp.Sexp.List _ [(Pa_ppx_located_sexp.Sexp.Atom _ $str:jscid$) ; Pa_ppx_located_sexp.Sexp.List _ $listpat$] >>,
            <:expr< ` $cid$ $tup$ >>)
        ] in
  ...
  let lefts = List.map (mustLeft "of_located_sexp") lefts in
...
and fmt_record ~{allow_extra_fields} ~{cid} loc arg fields = 
  let labels_vars_fmts_omits_jskeys = List.map (fun (_, fname, _, ty, attrs) ->
        ...
        | Some _ -> failwith "@key attribute without string payload"
        | None -> fname ] in
        (fname, Printf.sprintf "v_%s" fname, fmtrec ty, omit, jskey)) fields in
  ...
  let branch1 i (f, v, fmt,omit, jskey) =
    ...
    [(<:patt< [ Pa_ppx_located_sexp.Sexp.List _ [ Pa_ppx_located_sexp.Sexp.Atom _ $str:jskey$ ; $lid:v$ ] :: xs] >>, <:vala< None >>,
  ...
  let finish_branch =
    let recexp =
      let lel = List.map (fun (f,v,_,_,_) -> (<:patt< $lid:f$ >>, <:expr< $lid:v$ >>)) labels_vars_fmts_omits_jskeys in
      <:expr< { $list:lel$ } >> in
    ...
    let e = List.fold_right (fun (_,v,_,_,_) rhs -> 
        <:expr< Rresult.R.bind $lid:v$ (fun $lid:v$ -> $rhs$) >>) labels_vars_fmts_omits_jskeys consexp in
    (<:patt< [] >>, <:vala< None >>, e) in
  ...
  let e = 
    let varpats = List.map (fun (_,v,_,_,_) -> <:patt< $lid:v$ >>) labels_vars_fmts_omits_jskeys in
    ...
    let initexps = List.map (fun (f,_,fmt,omit,_) ->
        ...
        | Omit {omit_nil=True; default=None} -> <:expr< Result.Ok ($fmt$ (Pa_ppx_located_sexp.Sexp.List Ploc.dummy [])) >>

        | Option -> <:expr< Result.Ok None >>

        | List -> <:expr< Result.Ok [] >>

        | Array -> <:expr< Result.Ok [||] >>

        | Bool -> <:expr< Result.Ok False >>
  ...
  let ty0 = match ty0 with [
        <:ctyp:< { $list:_$ } >> when None <> extract_allowed_attribute_expr arg ("located_sexp", "allow_extra_fields") attrs ->
...
value sig_item_top_funs arg td =
  ...
  let of_sexpfname = of_sexp_fname arg tyname in
  let paramtys = List.map (fun p -> <:ctyp< ' $PM.type_id p$ >>) param_map in
  let argfmttys = List.map (fun pty -> <:ctyp< Pa_ppx_located_sexp.Sexp.t -> $pty$ >>) paramtys in  
  let ty = <:ctyp< $lid:tyname$ >> in
  let offtype = Ctyp.arrows_list loc argfmttys <:ctyp< Pa_ppx_located_sexp.Sexp.t -> $(Ctyp.applist ty paramtys)$ >> in
...
value str_item_funs arg td =
  (if Ctxt.is_plugin_name arg "located_sexp_of" || Ctxt.is_plugin_name arg "located_sexp" then
     To.str_item_funs arg td
   else []) @
  (if Ctxt.is_plugin_name arg "of_located_sexp" || Ctxt.is_plugin_name arg "located_sexp" then
...
value expr_sexp arg = fun [
  <:expr:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "located_sexp_of" || id = "derive.sexp_of" ->
...
| <:expr:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "of_located_sexp" || id = "derive.of_located_sexp" ->
    let loc = loc_of_ctyp ty in
    let runtime_module = Base.module_expr_runtime_module <:module_expr< Runtime >> in
    let param_map = ty |> type_params |> Of.PM.make_of_ids in
    let e = Of.fmt_of_top ~{msg=Printf.sprintf "%s.of_located_sexp"  (Ctxt.module_path_s arg)} arg param_map (ty,[]) in
    let e = <:expr< let open! $runtime_module$ in let open! Stdlib in $e$ >> in
...
value ctyp_sexp arg = fun [
  <:ctyp:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "located_sexp_of" || id = "derive.sexp_of" ->
    let param_map = ty |> type_params |> To.PM.make_of_ids in
    let argfmttys = List.map (To.PM.arg_ctyp loc) param_map in  
    Ctyp.arrows_list loc argfmttys <:ctyp< $ty$ -> Pa_ppx_located_sexp.Sexp.t >>

| <:ctyp:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "of_located_sexp" || id = "derive.of_located_sexp" ->
    let param_map = ty |> type_params |> Of.PM.make_of_ids in
    let argfmttys = List.map (Of.PM.arg_ctyp loc) param_map in  
    Ctyp.arrows_list loc argfmttys <:ctyp< Pa_ppx_located_sexp.Sexp.t -> $ty$ >>
...
Pa_deriving.(Registry.add PI.{
  name = "located_sexp"
; alternates = ["of_located_sexp"; "located_sexp_of"; "located_sexp_poly"]
...
; alg_attributes = ["nobuiltin"; "key"; "name"; "encoding"; "default"
                    ...
                    ; "bool"
                    ; "allow_extra_fields"
                    ; "located_sexp_of"; "of_located_sexp"]
; expr_extensions = ["of_located_sexp"; "located_sexp_of"]
; ctyp_extensions = ["of_located_sexp"; "located_sexp_of"]
; expr = expr_sexp
; ctyp = ctyp_sexp

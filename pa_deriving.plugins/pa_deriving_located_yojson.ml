(**pp -syntax camlp5r *)
(* camlp5r *)
(* pa_deriving_located_yojson.ml,v *)
...
module PM = ParamMap(struct value arg_ctyp_f loc pty = <:ctyp< $pty$ -> Pa_ppx_located_yojson.Json.t >> ; end) ;

type attrmod_t = [ Nobuiltin ] ;

value to_yojson_fname arg tyname =
  if tyname = "t" then "to_located_yojson"
  else tyname^"_to_located_yojson"
;
...
| <:ctyp:< Pa_ppx_located_yojson.Json.t >> -> <:expr< fun x -> x >>
| <:ctyp:< unit >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.unit_to_located_yojson >>
| <:ctyp:< int >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.int_to_located_yojson >>
| <:ctyp:< bool >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.bool_to_located_yojson >>
| <:ctyp:< int32 >> | <:ctyp:< Int32.t >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.int32_to_located_yojson >>
| <:ctyp:< int64 >> | <:ctyp:< Int64.t >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.int64_to_located_yojson >>
| <:ctyp:< int64 [@ $attrid:(_, id)$ `string ; ] >> |
  ...
  when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "encoding" ->
    <:expr< fun x -> Pa_ppx_located_yojson.Runtime.Yojson.string_to_located_yojson (Int64.to_string x) >>

| (<:ctyp:< string >> | <:ctyp:< Stdlib.String.t >> | <:ctyp:< String.t >>) ->
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.string_to_located_yojson >>
| <:ctyp:< bytes >> -> <:expr< fun x -> Pa_ppx_located_yojson.Runtime.Yojson.string_to_located_yojson (Bytes.to_string x) >>
| <:ctyp:< char >> -> <:expr< fun x -> Pa_ppx_located_yojson.Runtime.Yojson.string_to_located_yojson (String.make 1 x) >>
| <:ctyp:< nativeint >> | <:ctyp:< Nativeint.t >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.nativeint_to_located_yojson >>
| <:ctyp:< nativeint [@ $attrid:(_, id)$ `string ; ] >> |
  <:ctyp:< Nativeint.t [@ $attrid:(_, id)$ `string ; ] >>
  when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "encoding" ->
    <:expr< fun x -> Pa_ppx_located_yojson.Runtime.Yojson.string_to_located_yojson (Nativeint.to_string x) >>
| <:ctyp:< float >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.float_to_located_yojson >>

| <:ctyp:< Hashtbl.t >> ->
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.hashtbl_to_located_yojson >>

| <:ctyp:< $t$ [@ $attrid:(_, id)$ ] >> when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "nobuiltin" ->
    fmtrec ~{attrmod=Some Nobuiltin} t

| <:ctyp:< $t$ [@ $attrid:(_, id)$ $exp:e$ ;] >> when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "to_located_yojson" ->
    e
...
| <:ctyp:< list $ty$ >> ->
  ...
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.list_to_located_yojson $fmt1$ >>

| <:ctyp:< array $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.array_to_located_yojson $fmt1$ >>

| (<:ctyp:< ref $ty$ >> | <:ctyp:< Stdlib.ref $ty$ >>) ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.ref_to_located_yojson $fmt1$ >>

| <:ctyp:< option $ty$ >> ->
  let fmt1 = fmtrec ty in
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.option_to_located_yojson $fmt1$ >>
...
| <:ctyp:< [ $list:l$ ] >> ->
  let branches = List.map (fun [
    ...
    let jscid = match extract_allowed_attribute_expr arg ("located_yojson", "name") (uv attrs) with [
    ...
    (conspat, <:vala< None >>, <:expr< (Ploc.dummy, `List [ (Ploc.dummy, `String $str:jscid$) ;  $body$ ]) >>)
  ...
  | <:constructor:< $uid:cid$ of $list:tyl$ $_algattrs:attrs$ >> ->
    ...
    (conspat, <:vala< None >>, <:expr< (Ploc.dummy, `List [ (Ploc.dummy, `String $str:jscid$) :: $liste$ ]) >>)
...
| <:ctyp:< [= $list:l$ ] >> ->
  let branches = List.map (fun [
    ...
    let liste = List.fold_right2 (fun f v liste -> <:expr< [$f$ $lid:v$ :: $liste$] >>)
        fmts vars <:expr< [] >> in
    let liste = <:expr< (Ploc.dummy, `List [(Ploc.dummy, `String $str:jscid$) :: $liste$]) >> in
...
| <:ctyp:< ( $list:tyl$ ) >> ->
    ...
    let liste = List.fold_right2 (fun f v liste -> <:expr< [$f$ $lid:v$ :: $liste$] >>)
        fmts vars <:expr< [] >> in
    let varpats = List.map (fun v -> <:patt< $lid:v$ >>) vars in
    <:expr< fun ($list:varpats$) -> (Ploc.dummy, `List ($liste$)) >>
...
| [%unmatched_vala] -> failwith "pa_deriving_located_yojson.to_expression"
| ty ->
  Ploc.raise (loc_of_ctyp ty) (Failure Fmt.(str "pa_deriving_located_yojson.to_expression: %a" pp_ctyp ty))
]
and fmt_record loc arg fields = 
  let labels_vars_fmts_defaults_jskeys = List.map (fun (_, fname, _, ty, attrs) ->
        ...
        let default = extract_allowed_attribute_expr arg ("located_yojson", "default") attrs in
        let key = extract_allowed_attribute_expr arg ("located_yojson", "key") attrs in
  ...
  (<:patt< { $list:pl$ } >>, <:expr< (Ploc.dummy, `Assoc $liste$) >>)
...
value sig_item_fun0 arg td =
  ...
  let param_map = PM.make "located_yojson" loc (uv td.tdPrm) in
  let tyname = uv tyname in
  let to_yojsonfname = to_yojson_fname arg tyname in
  let paramtys = List.map (fun p -> <:ctyp< ' $PM.type_id p$ >>) param_map in
  let argfmttys = List.map (fun pty -> <:ctyp< $pty$ -> Pa_ppx_located_yojson.Json.t >>) paramtys in  
  let ty = <:ctyp< $lid:tyname$ >> in
  let toftype = Ctyp.arrows_list loc argfmttys <:ctyp< $(Ctyp.applist ty paramtys)$ -> Pa_ppx_located_yojson.Json.t >> in
...
| <:str_item:< type $_lid:_$ $_list:_$ = $_$ == $_priv:_$ .. $_itemattrs:_$ >> 
 as z ->
    ...
    let (to_yojsonfname, toftype) = sig_item_fun0 arg td in
    let modname = Printf.sprintf "M_%s" to_yojsonfname in
    let msg1 = Printf.sprintf "%s: Maybe a [@@deriving located_yojson] is missing when extending the type " to_yojsonfname in
...
| <:str_item:< type $lilongid:t$ $list:params$ += $_priv:_$ [ $list:ecs$ ] $_itemattrs:_$ >> ->
    ...
    let param_map = PM.make "located_yojson" loc params in
    let ec2gc = fun [
      EcTuple _ gc -> [gc]
    | EcRebind _ _ _ _ -> []
    ] in
...
value of_yojson_fname arg tyname =
  if tyname = "t" then "of_located_yojson"
  else tyname^"_of_located_yojson"
;
...
| <:ctyp:< Pa_ppx_located_yojson.Json.t >> -> <:expr< fun x -> Result.Ok x >>
| <:ctyp:< unit >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.unit_of_located_yojson $str:msg$ >>
| <:ctyp:< int >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.int_of_located_yojson $str:msg$ >>
| <:ctyp:< bool >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.bool_of_located_yojson $str:msg$ >>
| <:ctyp:< int32 >> | <:ctyp:< Int32.t >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.int32_of_located_yojson $str:msg$ >>
| <:ctyp:< int64 >> | <:ctyp:< Int64.t >> -> <:expr< Pa_ppx_located_yojson.Runtime.Yojson.int64_of_located_yojson $str:msg$ >>
| <:ctyp:< int64 [@ $attrid:(_, id)$ `string ; ] >> |
  <:ctyp:< Int64.t [@ $attrid:(_, id)$ `string ; ] >>
  when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "encoding" ->
  <:expr< fun [
...
| <:ctyp:< Hashtbl.t >> ->
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.hashtbl_of_located_yojson >>

| <:ctyp:< $t$ [@ $attrid:(_, id)$ ] >> when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "nobuiltin" ->
    fmtrec ~{attrmod=Some Nobuiltin} ~{msg=msg} t

| <:ctyp:< $t$ [@ $attrid:(_, id)$ $exp:e$ ;] >> when Some id = DC.allowed_attribute (DC.get arg) "located_yojson" "of_located_yojson" ->
    e
...
| <:ctyp:< list $ty$ >> ->
  ...
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.list_of_located_yojson $str:msg$ $fmt1$ >>

| <:ctyp:< array $ty$ >> ->
  let fmt1 = fmtrec ~{msg=msg} ty in
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.array_of_located_yojson $str:msg$ $fmt1$ >>

| (<:ctyp:< ref $ty$ >> | <:ctyp:< Stdlib.ref $ty$ >>) ->
  let fmt1 = fmtrec ~{msg=msg} ty in
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.ref_of_located_yojson $fmt1$ >>

| <:ctyp:< option $ty$ >> ->
  let fmt1 = fmtrec ~{msg=msg} ty in
  <:expr< Pa_ppx_located_yojson.Runtime.Yojson.option_of_located_yojson $fmt1$ >>

| <:ctyp:< $t1$ $t2$ >> -> <:expr< $fmtrec ~{msg=msg} t1$ $fmtrec ~{msg=msg} t2$ >>

| <:ctyp:< '$i$ >> ->
  let p = match PM.find i param_map with [
    x -> x | exception Not_found -> failwith "pa_deriving.located_yojson: unrecognized param-var in type-decl"
  ] in
  PM.arg_expr loc p
...
| <:ctyp:< [ $list:l$ ] >> ->
  let branches = List.map (fun [
    ...
    let conspat = <:patt< (_, `List [ (_, `String $str:jscid$) ; $recpat$ ]) >> in
  ...
  | <:constructor:< $uid:cid$ of $list:tyl$ $_algattrs:attrs$ >> ->
    ...
    let conspat = List.fold_right (fun v rhs -> <:patt< [ $lid:v$ :: $rhs$ ] >>)
        vars <:patt< [] >> in
    let conspat = <:patt< (_, `List [ (_, `String $str:jscid$) :: $conspat$ ]) >> in
...
| <:ctyp:< [= $list:l$ ] >> as ty0 -> 
  ...
  let branches = List.map (fun [
    ...
    let listpat = List.fold_right (fun vp listpat -> <:patt< [ $vp$ :: $listpat$ ] >>)
        varpats <:patt< [] >> in
    let conspat = <:patt< (_, `List [(_, `String $str:jscid$) :: $listpat$]) >> in
  ...
  let lefts = List.map (mustLeft "of_located_yojson") lefts in
  let rights = List.map (mustRight "of_located_yojson") rights in
...
| ty ->
  Ploc.raise (loc_of_ctyp ty) (Failure Fmt.(str "pa_deriving_located_yojson.of_expression: %a" pp_ctyp ty))
]
...
value sig_item_fun0 arg td =
  ...
  let exnftype = Ctyp.arrows_list loc argfmttys <:ctyp< Pa_ppx_located_yojson.Json.t -> $(Ctyp.applist ty paramtys)$ >> in
...
| <:str_item:< type $_lid:_$ $_list:_$ = $_$ == $_priv:_$ .. $_itemattrs:_$ >> 
   as z ->
    ...
    let modname = Printf.sprintf "M_%s" of_yojsonfname in
    let msg1 = Printf.sprintf "%s: Maybe a [@@deriving located_yojson] is missing when extending the type " of_yojsonfname in
    let msg2 = td.tdNam |> uv |> snd |> uv in
...
value str_item_funs arg td =
  (if Ctxt.is_plugin_name arg "to_located_yojson" || Ctxt.is_plugin_name arg "located_yojson" then
     To.str_item_funs arg td
   else []) @
  (if Ctxt.is_plugin_name arg "of_located_yojson" || Ctxt.is_plugin_name arg "located_yojson" then
...
value expr_yojson arg = fun [
  <:expr:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "to_located_yojson" || id = "derive.to_located_yojson" ->
  let runtime_module =
    Base.module_expr_runtime_module <:module_expr< Runtime >> in
    let param_map = ty |> type_params |> To.PM.make_of_ids in
    let coercion = monomorphize_ctyp ty in
    let e = To.fmt_to_top arg ~{coercion=coercion} ~{msg=Printf.sprintf "%s.to_located_yojson"  (Ctxt.module_path_s arg)} param_map ty in
...
| <:expr:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "of_located_yojson" || id = "derive.of_located_yojson" ->
  let runtime_module =
    Base.module_expr_runtime_module <:module_expr< Runtime >> in
    let param_map = ty |> type_params |> Of.PM.make_of_ids in
    let e = Of.fmt_of_top ~{msg=Printf.sprintf "%s.of_located_yojson"  (Ctxt.module_path_s arg)} arg param_map ty in
    let e = <:expr< let open! $runtime_module$ in let open! Stdlib in $e$ >> in
...
value ctyp_yojson arg = fun [
  <:ctyp:< [% $attrid:(_, id)$: $type:ty$ ] >> when id = "to_located_yojson" || id = "derive.to_located_yojson" ->
    let param_map = ty |> type_params |> To.PM.make_of_ids in
    let argfmttys = List.map (To.PM.arg_ctyp loc) param_map in  
    Ctyp.arrows_list loc argfmttys <:ctyp< $ty$ -> Pa_ppx_located_yojson.Json.t >>

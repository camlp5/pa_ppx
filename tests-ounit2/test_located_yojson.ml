...
let good str =
  ...
  let located_yojson = Pa_ppx_located_yojson.Json.of_string str in
...
let bad ~yojson_msg ~msg str =
  ...
  Testutil.assert_raises_exn_pattern msg (fun _ -> Pa_ppx_located_yojson.Json.of_string str) ;
  ()
...
let test_equality ctxt =
  let e1 = Json.of_string "{}" in
  let e2 = Json.of_string " {}" in
